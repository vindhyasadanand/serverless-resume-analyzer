"""
NLP Analysis Engine - Group 20 Custom Implementation
Team: Keyur Modi (Lead - NLP), Naveen John (Testing), Vindhya Hegde (Integration)

Our proprietary scoring algorithm combines multiple NLP techniques:
1. TF-IDF vectorization (scikit-learn) with custom parameters
2. Cosine similarity calculation for semantic matching
3. Multi-factor scoring (skills, experience, education, format)
4. AI-powered recommendation generation

Algorithm Design:
- Preprocessing: Lowercase normalization, special character removal
- Vectorization: TF-IDF with bigrams (ngram_range=1,2)
- Scoring: Weighted category breakdown (0-100 scale)
- Recommendations: Context-aware suggestions based on score thresholds
"""
import re
import numpy as np
...
class ResumeAnalyzer:
    ...
    def _calculate_breakdown(
        ...
        Returns scores for skills, experience, education
        """
        # Skills score (based on matching percentage)
        if job_skills:
            skills_score = (len(matched_skills) / len(job_skills)) * 100
        else:
            skills_score = 50  # Neutral score if no specific skills in job desc
        
        # Experience score (based on presence and similarity)
        experience_score = 0
        if resume_data['experience']:
            experience_score = 70  # Base score for having experience
            exp_text = ' '.join(resume_data['experience'])
            if len(exp_text) > 100:
                experience_score += 20  # Bonus for detailed experience
        
        # Education score
        education_score = 0
        if resume_data['education']:
            education_score = 80  # Base score for having education
        ...
        # Overall section presence score
